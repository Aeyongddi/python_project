...
# 절대 경로 설정
BASE_DIR = r'D:\python_project'  # 프로젝트의 루트 디렉토리

# 모델 로드
@st.cache_resource
def load_prediction_model():
    model_path = os.path.join(BASE_DIR, 'model', 'kospi_prediction_model.h5')
...
@st.cache_data
def load_data():
    data_path = os.path.join(BASE_DIR, 'dataset', 'kospi_data.csv')
    data = pd.read_csv(
        data_path,
        skiprows=2,
        parse_dates=['Date'],
...
# 고정된 날짜
fixed_start_date = pd.Timestamp("2025-01-01").date()
max_date = pd.Timestamp("2025-12-31").date()

# 시작 날짜: 고정
start_date_future = fixed_start_date
st.write(f"**예측 시작 날짜**: {start_date_future}")

# 종료 날짜: 사용자 입력
end_date_future = st.date_input(
    "예측 종료 날짜",
    value=fixed_start_date + pd.Timedelta(days=1),
